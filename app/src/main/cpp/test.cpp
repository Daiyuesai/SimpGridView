...
JNIEXPORT void JNICALL Java_com_xiaozhanxiang_simplegridview_view_MyVideoView_render
    ...
    const char * avPath = env ->GetStringUTFChars(path, NULL);
    //第一步 注册
    av_register_all();
    // 2 获取视频内容
    ...
    char buff[] = "";
    //打开一个输入流
    if (error =  avformat_open_input(&avFormatContext,avPath,NULL,NULL) < 0){
        av_strerror(error,buff,1024);
        LOGE("Couldn't open file %s: %d(%s)", avPath, error, buff);
        // LOGE("%d",error)
        LOGE("打开视频失败");
        return;
    }
    ...
    //缓存区
    uint8_t *out_buffer = (uint8_t *)(av_malloc(sizeof(avpicture_get_size(AV_PIX_FMT_RGBA,
...
//解锁
    ...
    env->ReleaseStringUTFChars(path, avPath);
}
...
//JNIEXPORT void JNICALL Java_com_xiaozhanxiang_simplegridview_view_VideoView_render
//        (JNIEnv * env, jobject thiz, jstring path,jobject surface){
//
//    const char * avPath = env ->GetStringUTFChars(path, NULL);
//    //第一步 注册
...
//    // 2 获取视频内容
//    AVFormatContext * avFormatContext = avformat_alloc_context(); //获取上下文
//    avformat_open_input(&avFormatContext,avPath,NULL,NULL); //打开一个输入流
//    avformat_find_stream_info(avFormatContext,NULL);
//
//    //我们应当找出相对应的视频流
//    int video_index = -1;
...
//    AVFrame *rgb_frame = av_frame_alloc();//分配一个AVFrame结构体，指向存放转换成rgb后的帧
//    //缓存区
//    uint8_t *out_buffer = static_cast<uint8_t *>(av_malloc(sizeof(avpicture_get_size(AV_PIX_FMT_RGBA,
//                                                                                     avCodecContext->width, avCodecContext ->height))));
//    //与缓存区相关联，设置rgb_frame缓存区
//    avpicture_fill(reinterpret_cast<AVPicture *>(rgb_frame), out_buffer, AV_PIX_FMT_RGBA, avCodecContext->width, avCodecContext->height);
//
//    SwsContext* swsContext = sws_getContext(avCodecContext->width,avCodecContext->height,avCodecContext->pix_fmt,
//                                            avCodecContext->width,avCodecContext->height,AV_PIX_FMT_RGBA,
//                                            SWS_BICUBIC,NULL,NULL,NULL);
